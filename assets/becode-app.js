...
function createObject(nombre = tabNumRow) {
	// déclaration tab vide
	let tabNew = [];
	let tabFina = [];
	// boucle pour le tour de mon tab1
	for (let x = 1; x < nombre; x++) {
		tabNew = [];
	...
	drawChart(tabFina)// crée le contenue du graphique
...
/* Graphique **************************** */
// fonction qui crée le SVG dans le DOM
function makeDraw() {
	var width = 500;
	var height = 300;
	var margin = 0;
	...
	var svg = d3.select("#bodyContent").select("h3").append("svg")
		...
		.attr("style", "overflow:visible; margin-top:50px; margin-left:50px")
		.append('g')
		.attr("transform", `translate(${margin}, ${margin})`);

}
// fonction qui crée le contenu du SVG 
...
function makeBtn() {
	div = d3.select("#bodyContent").select("h3").append("div")
		.attr("style", "width: 100%")
	for (let i = 1; i < tabNumRow; i++) {
		name = dateYear = d3.selectAll("#table1>tbody").selectAll("tr")._groups[0][i].cells[1].innerHTML;
		div.insert("button")
			.attr("onclick", `clicker(${i})`)
			.text(`${name}`)
...
d3.select(".container").select("#table1")
